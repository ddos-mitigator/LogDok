    ...
    content_string = '\n'.join(
        f'''
            <tr>
                ...
                <td><a href="log?name={container['Name']}"> \
                {container['Status']}</a></td>
            </tr>'''
...
def get_containerslist():
    container_list = [
        {'Name': container["Names"][0][1:], 'Status': container["Status"]}
...
def get_log(query_string):
    if query_string:
        ...
        for query in query_list:
            ...
            try:
                if param_list[0] == 'name':
                    ...
                    tail = param_list[1]          \
                        if param_list[1] == 'all' \
                        else int(param_list[1])
            except (IndexError, ValueError):
                raise SiteQueryException(
                    f'for field {param_list[0]} value not set \
                    [or incorrect]\n'.encode())
        ...
        try:
            ...
            raise SiteQueryException(
                f'logs not found for {container_name}, tail {tail}\n'.encode())
